...
// Tint mask: a second texture, over the same coordinates, whose red,
// green and blue are how much of each pixel belongs to material slots
// 0, 1 and 2. Each zone takes its slot's tint, so one texture can be
// recoloured per object without duplicating the image. Weights, not
// labels, because the lookup filters: halfway between two zones is half
// of each, which is also what the eye expects to see there.
uniform sampler2D tintMask;
uniform float     hasTintMask;   // 0 = no mask, the texture as drawn
// Alpha cutout: the texture's alpha is coverage, and a pixel whose
// filtered alpha is under ALPHA_CUTOUT_THRESHOLD is not drawn. Off, the
// alpha decides nothing.
uniform float     alphaCutout;
// Half: the filtered edge of a hole crosses it halfway between the last
// covered texel and the first empty one, so the cut lands where the
